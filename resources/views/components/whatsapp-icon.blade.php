{{--
    Shared WhatsApp glyph. Used by the CTA section and the runner directory so the
    path data lives in exactly one place. Usage: <x-whatsapp-icon class="h-4 w-4" />
--}}
<svg {{ $attributes->merge(['class' => 'h-4 w-4']) }} fill="currentColor" viewBox="0 0 24 24" aria-hidden="true">
